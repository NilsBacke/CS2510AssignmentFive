...
class Blob {
  ...
  /**
   * move this blob 5 pixels in the direction given by the ke or change its color
   * to Green, Red or Yellow
   */
  ...
  Blob randomMove(int n) {
    return new Blob(new Posn(this.center.x + this.randomInt(n), this.center.y + this.randomInt(n)),
  ...
  boolean nearCenter(int width, int height) {
    return this.center.x > width / 2 - 10 && this.center.x < width / 2 + 10
        && this.center.y > height / 2 - 10 && this.center.y < height / 2 + 10;
  }
}
...
class BlobWorldFun extends World {
  ...
  public WorldImage blackHole = new OverlayImage(
      new CircleImage(10, OutlineMode.SOLID, Color.BLACK),
      new RectangleImage(this.width, this.height, OutlineMode.SOLID, Color.BLUE));

  /**
   * produce the image of this world by adding the moving blob to the background
   * image
   */
  public WorldScene makeScene() {
    return this.getEmptyScene().placeImageXY(this.blackHole, this.width / 2, this.height / 2)
        .placeImageXY(this.blob.blobImage(), this.blob.center.x, this.blob.center.y);
  }
  ...
  public WorldEnd worldEnds() {
    ...
    if (this.blob.nearCenter(this.width, this.height)) {
      return new WorldEnd(true, this.makeScene().placeImageXY(
          new TextImage("Black hole ate the blob", 13, FontStyle.BOLD_ITALIC, Color.red), 100, 40));
    }
    else {
...
class BlobExamples {
  ...
  BlobWorldFun b1mouse50x50w = new BlobWorldFun(new Blob(new Posn(50, 50), 20, Color.RED));

  BlobWorldFun bwOutOfBounds = new BlobWorldFun(new Blob(new Posn(100, 350), 50, Color.RED));

  BlobWorldFun bwInTheCenter = new BlobWorldFun(new Blob(new Posn(100, 150), 50, Color.RED));
  ...
  boolean testOutsideBounds(Tester t) {
    return t.checkExpect(this.b1.outsideBounds(60, 200), true, "test outsideBounds on the right") &&

        t.checkExpect(this.b1.outsideBounds(100, 90), true, "test outsideBounds below") &&

        t.checkExpect(new Blob(new Posn(-5, 100), 50, Color.RED).outsideBounds(100, 110), true,
            "test outsideBounds above")
        &&

        t.checkExpect(new Blob(new Posn(80, -5), 50, Color.BLUE).outsideBounds(100, 90), true,
            "test outsideBounds on the left")
        &&

        t.checkExpect(this.b1.outsideBounds(200, 400), false, "test outsideBounds - within bounds");
  }
  ...
  boolean testRandomMove(Tester t) {
    return t.checkOneOf("test randomMove", this.b1.randomMove(1),
        new Blob(new Posn(99, 99), 50, Color.RED), new Blob(new Posn(99, 100), 50, Color.RED),
        new Blob(new Posn(99, 101), 50, Color.RED), new Blob(new Posn(100, 99), 50, Color.RED),
        new Blob(new Posn(100, 100), 50, Color.RED), new Blob(new Posn(100, 101), 50, Color.RED),
        new Blob(new Posn(101, 99), 50, Color.RED), new Blob(new Posn(101, 100), 50, Color.RED),
        new Blob(new Posn(101, 101), 50, Color.RED));
  }
  ...
  /**
   * test the method onTick when the world should end in the BlobWorldFun class
   */
  boolean testOnTick2(Tester t) {
    return
    // to test the world ending, verify the value of the lastWorld
    t.checkExpect(this.bwOutOfBounds.testOnTick().lastWorld,
        new WorldEnd(true, this.bwOutOfBounds.lastScene("Blob is outside the bounds"))) &&
  ...
  boolean testWorldEnds(Tester t) {
    return t
        .checkExpect(this.bwOutOfBounds.worldEnds(),
            new WorldEnd(true,
                this.bwOutOfBounds.makeScene()
                    .placeImageXY(new TextImage("Blob is outside the bounds", Color.red), 100, 40)))
        &&
        ...
        t.checkExpect(this.b1w.worldEnds(), new WorldEnd(false, this.b1w.makeScene()));
  }

  /** run the animation */
  BlobWorldFun w1 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, Color.RED));
  BlobWorldFun w2 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, Color.RED));
  BlobWorldFun w3 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, Color.RED));

  // test that we can run three different animations concurrently
  // with the events directed to the correct version of the world
  /*
   * boolean runAnimation = this.w1.bigBang(200, 300, 0.3); boolean runAnimation2
   * = this.w2.bigBang(200, 300, 0.3); boolean runAnimation3 =
  ...
  public static void main(String[] argv) {
    ...
    // run the game
    BlobWorldFun w = new BlobWorldFun(new Blob(new Posn(150, 100), 20, Color.RED));
     ...
     * CircleImage(new Posn(50, 50), 20, Color.RED), new RectangleImage(new Posn(20,
     * 30), 40, 20, Color.BLUE)));
     */
  }
